...
export class AssetAdvancedTools implements ToolExecutor {
    ...
    private async readAssetContent(url: string): Promise<ToolResponse> {
        return new Promise(async (resolve) => {
            try {
                ...
                try {
                    ...
                    console.warn('asset-db read-asset API not available, falling back to file system read:', (err as Error).message);
                }
